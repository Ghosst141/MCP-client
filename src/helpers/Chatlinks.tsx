...
function Chatlinks({ id,title, selected, onSelect }: ChatlinksProps) {
    ...
    return (
        <div className={`chat-links ${selected ? 'selected' : ''}`} onClick={onSelect}>
            <div className='chat-title' onClick={handleClick}><span>{title}</span></div>
            <div className='chat-link-btn'>
